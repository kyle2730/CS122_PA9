...
void puck_slide(sf::CircleShape& puck, const sf::CircleShape& mallet, const sf::CircleShape& player) {

    int wall = 0;
    float multiplier = 1;
    static int buffer = 0;
    static float x_speed = 0, y_speed = 0, x_player_pos = 0, y_player_pos = 0, x_mallet_pos = 0, y_mallet_pos = 0;
    sf::Vector2f slide(x_speed, y_speed);
    sf::Vector2f mallet_last_pos(x_mallet_pos, y_mallet_pos);
    sf::Vector2f player_last_pos(x_player_pos, y_player_pos);

    //moves puck by slide vector amount
    puck.move(slide);

    //if puck touches mallets direction is altered
    //cannot register mallot touch within 100 iterations
    //slide speed is determined by speed of mallet and speed of puck
    if (touching_hitdisc(puck, mallet) && (buffer == 0)) {
        multiplier = magnitude(slide - (mallet.getPosition() - mallet_last_pos));
        slide = normal_direction(mallet, puck) * multiplier;
        buffer = 1000;
    }
    if (touching_hitdisc(puck, player) && (buffer == 0)) {
        multiplier = magnitude(slide - (player.getPosition() - player_last_pos) * 0.02f);
        if (multiplier > 0.4) multiplier = 0.4;
        slide = normal_direction(player, puck) * multiplier;
        buffer = 1000;
    }

    //checks if puck hit wall
    wall = hit_window(puck, 1);

    //if puck hits wall, direction is reversed
    if (wall != 0) {
        ...
        if (((wall >> 3) % 2) || ((wall >> 2) % 2)) {
            slide.y *= -1.0f;
        }
    }

    //puck speed slows down gradually
    if (fabsf(slide.x) < 0.001 && fabsf(slide.y < 0.001)) slide *= 0.0f;
    else {
        slide.x *= 0.9999f;
        slide.y *= 0.9999f;
    }

    //records current speed of puck
    x_speed = slide.x;
    y_speed = slide.y;

    //records current position of mallets
    x_mallet_pos = mallet.getPosition().x;
    y_mallet_pos = mallet.getPosition().y;
    x_player_pos = player.getPosition().x;
    y_player_pos = player.getPosition().y;

    //runs through buffer time
    if (buffer != 0) buffer--;
}
